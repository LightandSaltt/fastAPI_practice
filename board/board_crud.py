...
from board.board_schema import NewPost, Post_List, Post, Update_Post
...
def list_all_post(db: Session):
    ...
    return [Post_List(no=row.no, writer=row.writer, title=row.title, date=row.date) for row in lists]

def get_post(post_no: int, db: Session):
    try:
        post = db.query(Board).filter(and_(Board.no == post_no, Board.del_yn == 'Y')).first()
        return Post(no=post.no, writer=post.writer, title=post.title, conent=post.content, date=post.date)
    except Exception as e:
        return {'error': str(e), 'msg': '존재하지 않는 게시글 번호입니다.'}

def update_post(update_post: Update_Post, db: Session):

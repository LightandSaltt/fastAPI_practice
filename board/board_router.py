...
async def creat_new_post(new_post: board_schema.NewPost, db: Session = Depends(get_db)):
...
async def update_post(update_post: board_schema.Update_Post, db: Session = Depends(get_db)):
